...
class readTime {
    ...
    /**
     * @property {number} codeBlockCount
     ...
     * @description estimated time to read the document in seconds. Preset assumes that the reader will take time to adjust as the webpage loads.
     * @memberof readTime
     */
    time = 5;
     ...
     * @memberof readTime
     */
    _commentsRegEx =/(```[\s\S]*?```|`[\s\S]*?`|<!--[\s\S]*?-->)|(<!--[\s\S]*?-->)/gm;
    ...
    /**
     * @property {string} _codeBlockRegex
     * @description regex to match video tags in the HTML, markdown text, and HEXO
     * @private
     * @memberof readTime
     */
    _codeBlockRegex = /```([\s\S]*?)```|<code\b[^>]*>([\s\S]*?)<\/code>/gi;
    ...
    /**
     * @method codeBlocksCounter
     * @desc count encapsulated characters and number of code blocks
     * @returns {number} cbCharCount - total codeblock characters
     * @private
     * @memberof readTime
     */
    codeBlocksCounter() {

        let match, cbCharCount = 0;

        while ((match = this._codeBlockRegex.exec(this.text)) !== null) {
          let codeBlockContent = match[0] || match[1];
          this.codeBlockCount++;
          if (codeBlockContent !== undefined) cbCharCount += codeBlockContent.length;
        }
    ...
    filterHiddenComments() {
        ...
        this.text.replace(this._commentsRegEx, (match, p1, p2) => p1 ? p1 : '');
    }
    ...
    calculate() {
        ...
        this.time += Math.ceil(this.codeBlocksCounter() / this.langProfile.charPerMin) * 2;
