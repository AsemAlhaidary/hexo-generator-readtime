...
class readTime {
     ...
     * @description number of `{% video|vimeo|youtube %}` tags found in document
     */
    vidCount = 0;
    ...
    /**
     * @function imgCounter
     * @desc count number of image tags in the HTML, markdown text, and HEXO tags `{% img [...] %}`, `{% image [...] %}`, and `{% inlineimage [...] %}` tags.
     * @private
     */
    imgCounter() {

        const imgRegex = /<img[^>]+>|<image[^>]+>|!\[(.*?)\]\((.*?)\)|\{\%\s*(?:img|image|inlineimage)\s+[^%]*\%\}/gi;
    ...
    vidCounter() {
        ...
        const vidRegex = /<video[^>]+>|<object[^>]+>|!\[(.*?)\]\((.*?)\)|\{\%\s*(?:video|videojs|vimeo|youtube|youtuber)\s+[^%]*\%\}/gi;
        const vidMatches = this.text.match(vidRegex);
