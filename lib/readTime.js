...
class readTime {
     ...
     * @description regex to match word boundaries; Works only for Latin-based languages where a space is the word delimiter.
     * @private
     */
    _wordRegEx = /\b\S+\b/g;
    ...
    countCharacters() {
        ...
        let _RE = new RegExp("[" + this.langProfile.unicodeRange.join('') + "]", "gu");
        let matches = this.text.match(_RE) || [];
        this.charCount = this.text.length - matches.length;
    }

    /**
     * @function countWords
     * @desc Counts the number of words in `this.text`. Matches word boundaries while excluding tabs, newlines, and uncommon whitespace characters.
     *       
     * Note: supports UTF-8
     * @private
     */
    ...
    calculate() {
        ...
        // add word (in minutes) with image times (in seconds) + video times (in seconds) 
        this.time = Math.round((this.wordCount * 60 / this.langProfile.wordsPerMin) + this.imgTime() + this.vidTime());
