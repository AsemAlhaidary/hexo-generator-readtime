...
let isLangInSet = (readtimeSet, lang) => {
  return Object.keys(readtimeSet.langProfile).includes(lang);
}
...
let isPropInLang = (readtimeSet, prop, lang) => {
  return Object.keys(readtimeSet.langProfile[lang]).includes(prop);
}
...
let getWordsPerMin = (themeConfig, language, defaultSettings) => {
  ...
  if (isPropInSet(themeConfig.readtime, 'wordsPerMin', language)) {
    wordsPerMin = themeConfig.readtime.langProfile[language].wordsPerMin;

    // Is "wordsPerMin" property in defaultSettings at the wanted lang?
  } else if (isPropInSet(defaultSettings, 'wordsPerMin', language)) {
    wordsPerMin = defaultSettings.langProfile[language].wordsPerMin;

    // Use the "wordsPerMin" value of "en" lang
  } else {
    wordsPerMin = defaultSettings.langProfile.en.wordsPerMin;
  }
 ...
 * @private
 * @desc Calculate the time based on the given {timeUnit}. Employs "fall-through" or "cascade" logic. This returns more precise timeCount whereas getAutoFuzzyTime is rough-ballpark estimate.
 * @param {Number} timeCount - The time in (seconds)
 ...
 * @return {String} 
 */
let getPluralStr = (time, unit, language) => {
  let isPlural = unit[unit.length - 1] == 's';
  time = Math.round(time); // make time a decimal
  ...
  if (language == 'ar') {
    ...
    if (time > 10) {
      unit = unit.slice(0, unit.length - 1); // singular
    }
  }
...
let getFuzzyAbout = (hexo, defaultSettings, language) => {
  ...
  try {
    ...
    fuzzyAbout = hexo.theme.config.readtime.langProfile[language].fuzzyTime.about;
  } catch (error) {
    try {
      // Try to get the word from the default settings
      fuzzyAbout = defaultSettings.langProfile[language].fuzzyTime.about;
    } catch (error) {
...
let getPattern = (hexo, defaultSettings, language) => {
  ...
  try {
    ...
    pattern = hexo.theme.config.readtime.langProfile[language].fuzzyTime.pattern;
  } catch (error) {
    // Try to get the pattern from the default settings
    pattern = defaultSettings.langProfile[language].fuzzyTime.pattern;
  }
...
let getFuzzyTimePeriod = (hexo, defaultSettings, language, timeUnit) => {
  ...
  try {
    ...
    timePeriod = hexo.theme.config.readtime.langProfile[language].fuzzyTime.time_period[timeUnit];
  } catch (error) {
    try {
      // Try to get the timePeriod from the default settings
      timePeriod = defaultSettings.langProfile[language].fuzzyTime.time_period[timeUnit];
    } catch (error) {
 ...
 */
let readTimeLogic = (post, hexo, defaultSettings) => {
  ...
  let language = getPostLang(post, hexo);
  ...
  timeUnit = getPluralStr(timeCount, timeUnit, language);
